...
Meteor.methods({
  addChat(receiverId: string): void {
    ...
    const chat = { memberIds: [this.userId, receiverId] }

    Chats.insert(chat)
  },
  addMessage(type: MessageType, chatId: string, content: string) {
    if (!this.userId) throw new Meteor.Error('unauthorized', 'User must be logged-in to create a new chat')
    ...
    if (!chatExists) {
      throw new Meteor.Error('chat-not-exists', 'Chat doesnt exist')
    }
